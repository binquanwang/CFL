...
parser.add_argument('--mm', type=float, default=0.8, help=' the momentum of the memory')
...
def main():
    ...
    model_ema = net(class_num=class_num)
    model_ema = torch.nn.DataParallel(model_ema).cuda()
    model_ema.load_state_dict(pretrained_model)
...
def train(train_loader, model,model_ema, pv_criterion, pc_criterion, optimizer, centers, patch_centers, epoch):
    ...
    for i, (input, target, path, cams) in enumerate(train_loader):
       ...
       # print(patch_agent, position)
        ...
        _update_ema_variables(model, model_ema)
...
def _update_ema_variables(model, ema_model, alpha=0.01):
