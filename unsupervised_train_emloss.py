...
parser.add_argument('--eloss', default=1, type=float, help='the weight of the PEDAL loss')
...
def main():
    ...
    pe_criterion = loss.Emdal(scale=args.scale).cuda()
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        train(train_loader, model, pv_criterion, pc_criterion,pe_criterion, optimizer, centers, patch_centers, epoch)
...
def train(train_loader, model, pv_criterion, pc_criterion,pe_criterion, optimizer, centers, patch_centers, epoch):
    ...
    for i, (input, target, path, cams) in enumerate(train_loader):
       ...
       # patch_agent, position = patch_centers.get_soft_label(path, feat_list,epoch)
      ...
      #  print(feat.size(),patch_agent.size(),len(feat_list))
        if args.eloss != 0:
          #  feat = torch.stack(feat_list, dim=0)
           # feat_em = patch_agent[:,::args.num,:]
            eloss = pe_criterion(patch_agent, patch_agent,position,epoch)
        else:
            eloss = torch.tensor([0.]).cuda()
        ...
        total_loss =  args.iloss*iloss + args.ploss*ploss + args.eloss*eloss 


        i_losses.update(iloss.item(), input.size(0))
        p_losses.update(ploss.item(), input.size(0))
        e_losses.update(eloss.item(), input.size(0))
        ...
        if i % args.print_freq == 0:
            print('Epoch: [{0}][{1}/{2}]\t'
                  ...
                  'emdal {emdal.val:.4f} ({emdal.avg:.4f})\t'
                  'ipfl {ipfl.val:.4f} ({ipfl.avg:.4f})'.format(
                epoch, i, len(train_loader), batch_time=batch_time,
                data_time=data_time, ipfl=i_losses,emdal=e_losses, pedal=p_losses))
